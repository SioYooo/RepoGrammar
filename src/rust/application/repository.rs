...
use crate::application::recovery::{
    classify_recovery, recovery_command as authoritative_recovery_command, RecoveryAction,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum RepositoryReadinessState {
    NotInitialized,
    StateOnlyNoActiveIndex,
    ReadyActiveIndex,
    ActiveIndexUnhealthy,
    ActiveIndexStaleOrUnreadable,
    AutosyncRecommended,
    AutosyncActive,
    StorageUnhealthy,
    Unknown,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RepositoryAutosyncReadiness {
    pub configured: bool,
    pub running: bool,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ForeignProviderState {
    pub name: String,
    pub path: String,
    pub present: bool,
    pub managed_by_repogrammar: bool,
    pub tracked_risk: Option<bool>,
    pub recommendation: String,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LocalStateHygiene {
    pub repogrammar_state_present: bool,
    pub repogrammar_state_ignored: Option<bool>,
    pub repogrammar_state_tracked_risk: Option<bool>,
    pub repogrammar_recommendation: Option<String>,
    pub foreign_provider_state: Vec<ForeignProviderState>,
}
...
pub struct RepositoryReadiness {
    ...
    pub active_generation_available: bool,
    pub recommended_next_command: Option<String>,
    pub requires_user_permission: bool,
    pub autosync: RepositoryAutosyncReadiness,
    pub local_state_hygiene: LocalStateHygiene,
    /// Transport-neutral decision retained for other application consumers.
    /// CLI JSON serializers intentionally keep their existing public shape.
    pub recovery: Option<RecoveryRecommendation>,
}

impl Default for RepositoryReadiness {
    fn default() -> Self {
        Self {
            ...
            autosync: RepositoryAutosyncReadiness {
                configured: false,
                running: false,
                recommended: false,
            },
            local_state_hygiene: LocalStateHygiene {
                repogrammar_state_present: false,
                repogrammar_state_ignored: None,
                repogrammar_state_tracked_risk: None,
                repogrammar_recommendation: None,
                foreign_provider_state: Vec::new(),
            },
            recovery: None,
        }
    }
...
impl RepositoryStatusReport {
    fn with_readiness(mut self, resolved: &ResolvedStateDir) -> Self {
        self.readiness = repository_readiness(resolved, &self);
        self
    }
}
    ...
    match status.indexing {
        ...
        RepositoryImplementationStatus::SyntaxOnlyCodeUnits => {
            findings.push(RepositoryDoctorFinding {
                ...
                detail: "syntax-only code units are stored; query conclusions remain gated by active family evidence and typed UNKNOWN results".to_string(),
            })
        }
...
fn classify_index_lock(lock: &IndexLockMetadata) -> IndexLockState {
    ...
    match process_liveness_for_lock(lock.pid, Some(lock.started_unix_seconds)) {
        ProcessLiveness::Live => IndexLockState::Active,
        ProcessLiveness::Dead => IndexLockState::Stale,
        ProcessLiveness::Unknown => IndexLockState::Unknown,
    }
}
...
fn status_for_resolved_state(
    ...
    if !resolved.absolute.exists() {
        return Ok(RepositoryStatusReport {
            ...
            storage_error: None,
            readiness: empty_repository_readiness(),
        }
        .with_readiness(resolved));
    }
    ensure_state_path_can_be_directory(&resolved.absolute)?;
...
fn repository_readiness(
    resolved: &ResolvedStateDir,
    report: &RepositoryStatusReport,
) -> RepositoryReadiness {
    let active_generation_available = report
        .storage_inspection
        .as_ref()
        .and_then(|inspection| inspection.active_generation.as_ref())
        .is_some();
    let local_state_hygiene = local_state_hygiene(resolved);
    let autosync = autosync_readiness(&resolved.absolute, active_generation_available);
    let context = repository_recovery_context(
        report,
        active_generation_available,
        &autosync,
        recovery_lock_state(&resolved.absolute),
    );
    let recovery = classify_recovery(&context);
    let query_ready = matches!(
    ...
    let state = readiness_state_from_recovery(recovery, &autosync);
    let recommended_next_command = recovery_command(recovery.action).map(str::to_string);
    let requires_user_permission = matches!(
        recovery.action,
...
fn repository_recovery_context(
    report: &RepositoryStatusReport,
    active_generation_available: bool,
    autosync: &RepositoryAutosyncReadiness,
    lock_state: RecoveryLockState,
) -> RecoveryContext {
    ...
    let freshness = repository_freshness_for_report(report, active_generation_available);
    ...
    RecoveryContext {
        ...
        autosync: RecoveryAutosyncState {
            ...
            recommended: autosync.recommended,
        },
        agent: RecoveryAgentState::NotRequired,
    }
}

pub(crate) fn repository_freshness_for_report(
    report: &RepositoryStatusReport,
    active_generation_available: bool,
) -> RecoveryFreshness {
    if !active_generation_available {
        return RecoveryFreshness::NotApplicable;
    }
    match report
        .storage_inspection
        .as_ref()
    ...
    {
        Some(0) => RecoveryFreshness::Fresh,
        Some(_) => RecoveryFreshness::Stale,
        None => RecoveryFreshness::CannotVerify,
    }
}

/// Return the repository recovery decision already attached by status/doctor.
///
/// The fallback exists only for transport/test runtimes that construct the
/// legacy report shape directly. It still enters the authoritative classifier
/// here rather than asking each interface consumer to interpret raw fields.
pub(crate) fn repository_recovery_for_report(
    report: &RepositoryStatusReport,
) -> RecoveryRecommendation {
    if let Some(recovery) = report.readiness.recovery {
        return recovery;
    }
    ...
    classify_recovery(&repository_recovery_context(
        report,
        active_generation_available,
        &report.readiness.autosync,
        RecoveryLockState::Clear,
    ))
}

fn recovery_lock_state(state_dir: &Path) -> RecoveryLockState {
...
fn readiness_state_from_recovery(
    recovery: RecoveryRecommendation,
    autosync: &RepositoryAutosyncReadiness,
) -> RepositoryReadinessState {
    match recovery.action {
        RecoveryAction::Setup => RepositoryReadinessState::NotInitialized,
        RecoveryAction::Resync if recovery.reason == RecoveryReason::ActiveIndexMissing => {
            RepositoryReadinessState::StateOnlyNoActiveIndex
        }
        RecoveryAction::Resync => RepositoryReadinessState::ActiveIndexStaleOrUnreadable,
        RecoveryAction::RepairStorage => RepositoryReadinessState::StorageUnhealthy,
        RecoveryAction::ResolveLock => RepositoryReadinessState::ActiveIndexUnhealthy,
        RecoveryAction::StartAutosync if recovery.reason == RecoveryReason::AutosyncRecommended => {
            RepositoryReadinessState::AutosyncRecommended
        }
        RecoveryAction::StartAutosync => RepositoryReadinessState::ActiveIndexStaleOrUnreadable,
        RecoveryAction::None if autosync.running => RepositoryReadinessState::AutosyncActive,
        RecoveryAction::None => RepositoryReadinessState::ReadyActiveIndex,
        RecoveryAction::UseSourceFallback | RecoveryAction::Unsupported => {
            RepositoryReadinessState::ActiveIndexStaleOrUnreadable
        }
        RecoveryAction::InstallAgent(_)
        | RecoveryAction::InstallSupportedAgent
        | RecoveryAction::RepairAgentIntegration(_) => RepositoryReadinessState::Unknown,
    }
}

fn recovery_command(action: RecoveryAction) -> Option<&'static str> {
    match action {
        RecoveryAction::UseSourceFallback | RecoveryAction::Unsupported | RecoveryAction::None => {
            None
        }
        _ => Some(authoritative_recovery_command(action)),
    }
}

fn autosync_readiness(
    state_dir: &Path,
    active_generation_available: bool,
) -> RepositoryAutosyncReadiness {
    let configured = autosync_config_enabled(state_dir);
    let running = configured && autosync_daemon_running(state_dir);
    RepositoryAutosyncReadiness {
        configured,
        running,
        recommended: active_generation_available && configured && !running,
    }
}

fn autosync_config_enabled(state_dir: &Path) -> bool {
    let path = state_dir.join("autosync.json");
    let Ok(contents) = read_lifecycle_text(&path) else {
        return false;
    };
...
fn autosync_daemon_running(state_dir: &Path) -> bool {
    let path = state_dir.join("locks").join("daemon.lock");
    let Ok(contents) = read_lifecycle_text(&path) else {
        return false;
    };
    let Ok(value) = serde_json::from_str::<serde_json::Value>(&contents) else {
    ...
    if value
        ...
        .and_then(serde_json::Value::as_u64)
        != Some(1)
        || value.get("kind").and_then(serde_json::Value::as_str) != Some("autosync_daemon")
    {
        return false;
    }
    ...
    let started_unix_seconds = value
        .get("started_unix_seconds")
        .and_then(serde_json::Value::as_u64);
    autosync_daemon_process_liveness(pid, started_unix_seconds) == ProcessLiveness::Live
}

fn local_state_hygiene(resolved: &ResolvedStateDir) -> LocalStateHygiene {
    let repogrammar_state_present = resolved.absolute.exists();
    let repogrammar_pathspec = dir_pathspec(&resolved.relative);
    let mut repogrammar_state_ignored = None;
    let mut repogrammar_state_tracked_risk = None;
    let mut foreign_provider_state = Vec::new();

    if let Ok(context) = GitContext::resolve(&resolved.root) {
        repogrammar_state_ignored = context.check_ignore_policy(&repogrammar_pathspec).ok();
        repogrammar_state_tracked_risk = context
            .has_tracked_entries_under(&repogrammar_pathspec)
            .ok();

        let codegraph_pathspec = ".codegraph/";
        let codegraph_present = resolved.root.join(".codegraph").exists();
        let codegraph_tracked_risk = context.has_tracked_entries_under(codegraph_pathspec).ok();
        if codegraph_present || codegraph_tracked_risk == Some(true) {
            foreign_provider_state.push(ForeignProviderState {
                name: "codegraph".to_string(),
                path: codegraph_pathspec.to_string(),
                present: codegraph_present,
                managed_by_repogrammar: false,
                tracked_risk: codegraph_tracked_risk,
                recommendation: "keep ignored or remove from index if accidentally tracked"
                    .to_string(),
            });
        }
    }

    let repogrammar_recommendation = if repogrammar_state_tracked_risk == Some(true) {
        Some("remove .repogrammar/ from the Git index and keep it ignored".to_string())
    } else if repogrammar_state_present && repogrammar_state_ignored == Some(false) {
        Some("repair Git exclude hygiene so .repogrammar/ stays untracked".to_string())
    } else {
        None
    };

    LocalStateHygiene {
        repogrammar_state_present,
        repogrammar_state_ignored,
        repogrammar_state_tracked_risk,
        repogrammar_recommendation,
        foreign_provider_state,
    }
}

fn dir_pathspec(relative: &str) -> String {
...
mod tests {
    ...
    #[test]
    fn status_and_doctor_share_authoritative_setup_resync_and_repair_recovery() {
        let workspace = TempWorkspace::new("repository-recovery-consumers");
        let root = root_string(workspace.path());

        let status = repository_status(RepositoryStatusRequest::new(root.clone()))
            .expect("status before init");
        assert_eq!(
            status.readiness.recovery.expect("status recovery").action,
            RecoveryAction::Setup
        );
        assert_eq!(
            status.readiness.recommended_next_command.as_deref(),
            Some("repogrammar setup")
        );
        let doctor = repository_doctor(RepositoryDoctorRequest::new(root.clone()))
            .expect("doctor before init");
        assert_eq!(doctor.status.readiness.recovery, status.readiness.recovery);
        ...
        let doctor =
            repository_doctor(RepositoryDoctorRequest::new(root)).expect("doctor after corruption");
        assert_eq!(
            doctor
                .status
                .readiness
                .recovery
                .expect("doctor recovery")
                .action,
            RecoveryAction::RepairStorage
        );
        assert_eq!(
            doctor.status.readiness.recommended_next_command.as_deref(),
            Some("repogrammar doctor")
        );
    }

    #[test]
    fn readiness_uses_classifier_for_autosync_without_losing_query_readiness() {
        ...
        let autosync = RepositoryAutosyncReadiness {
            configured: true,
            running: false,
            recommended: true,
        };
        let recovery = classify_recovery(&repository_recovery_context(
            &report,
            true,
            &autosync,
            RecoveryLockState::Clear,
        ...
        assert_eq!(recovery.action, RecoveryAction::StartAutosync);
        assert_eq!(
            readiness_state_from_recovery(recovery, &autosync),
            RepositoryReadinessState::AutosyncRecommended
        );
        assert_eq!(
            recovery_command(recovery.action),
            Some("repogrammar autosync start")
        );
    }

    #[test]
    fn doctor_does_not_claim_active_family_evidence_is_deferred() {
        let report = RepositoryStatusReport {
            state_dir: DEFAULT_STATE_DIR.to_string(),
            status: RepositoryStatus::Initialized {
                active_generation: "gen-000001".to_string(),
        ...
        let findings = doctor_findings_for_status(&report);
        let indexing = findings
            .iter()
            .find(|finding| finding.code == RepositoryDoctorCode::IndexingSyntaxOnlyCodeUnits)
            .expect("syntax-only doctor finding");
        assert!(!indexing
            .detail
            .contains("pattern-family evidence remain deferred"));
        assert!(indexing.detail.contains("active family evidence"));
        assert!(indexing.detail.contains("typed UNKNOWN"));
    }
...
"#,
    ...
    #[test]
    fn status_reports_local_state_hygiene_and_foreign_provider_tracked_risk() {
        let workspace = TempWorkspace::new("repository-readiness-hygiene-tracked");
        if !git_init(&workspace) {
            return;
        }
        init_repository(init_request(workspace.path())).expect("init repository");
        ...
        let add_status = Command::new("git")
            ...
            .current_dir(workspace.path())
            .status()
            .expect("git add tracked local state");
        ...
        let hygiene = &status.readiness.local_state_hygiene;

        assert!(hygiene.repogrammar_state_present);
        assert_eq!(hygiene.repogrammar_state_ignored, Some(true));
        assert_eq!(hygiene.repogrammar_state_tracked_risk, Some(true));
        assert!(hygiene
            .repogrammar_recommendation
            .as_deref()
            .expect("repogrammar recommendation")
            .contains("remove .repogrammar/ from the Git index"));
        let codegraph = hygiene
            .foreign_provider_state
            .iter()
            .find(|provider| provider.name == "codegraph")
            .expect("codegraph foreign provider state");
        assert_eq!(codegraph.path, ".codegraph/");
        assert!(codegraph.present);
        assert!(!codegraph.managed_by_repogrammar);
        assert_eq!(codegraph.tracked_risk, Some(true));
        assert!(codegraph
            .recommendation
            .contains("keep ignored or remove from index"));
    }
    ...
    fn unlock_force_yes_refuses_active_unknown_and_invalid_index_locks() {
        let cases = [
            ("active", current_index_lock_metadata().to_json(), "active"),
            (
                "unknown",
                IndexLockMetadata {
                    pid: 0,
    ...
    #[test]
    fn shared_process_liveness_identifies_current_process_and_dead_pid() {
        assert_eq!(
        ...
        assert_eq!(
            process_liveness_for_lock(definitely_dead_pid(), None),
    ...
    #[cfg(unix)]
    #[test]
    fn autosync_daemon_readiness_requires_daemon_command_line() {
        let workspace = TempWorkspace::new("repository-autosync-reused-pid");
        init_repository(init_request(workspace.path())).expect("init repository");
        let state_dir = workspace.path().join(DEFAULT_STATE_DIR);
        let lock_path = state_dir.join("locks").join("daemon.lock");
        fs::write(
            &lock_path,
            serde_json::json!({
                "schema_version": 1,
        ...
        assert!(!autosync_daemon_running(&state_dir));
    }
