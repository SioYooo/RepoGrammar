...
const STABLE_RELEASE_VERSION: &str = "0.4.3";
const STABLE_PREVIEW_VERSION: &str = "0.2.0-preview.0";
const STABLE_PREVIOUS_PUBLIC_VERSION: &str = "0.4.2";
...
const RELEASE_DEPENDENCY_URI: &str = "git+https://github.com/SioYooo/RepoGrammar@refs/tags/v0.4.3";
...
fn release_dist_tag_action(
    ...
    match channel {
        ...
        ReleaseChannel::Stable => {
            ...
            if !published_versions
                .iter()
                .any(|published| published == STABLE_PREVIOUS_PUBLIC_VERSION)
            {
                return Err("previous public stable version is not published");
            }
...
fn verify_stable_release_evidence(
    ...
    if candidate_run["name"] != "Release"
        ...
        || candidate_run["head_branch"] != "v0.4.3"
    ...
    for (name, expected) in [
        ("pinned-version.txt", "repogrammar 0.4.3"),
        ("latest-version.txt", "repogrammar 0.4.3"),
        ("preview-version.txt", "repogrammar 0.2.0-preview.0"),
        ("public-installer-version.txt", "repogrammar 0.4.3"),
    ] {
...
fn check_release_workflow_contract(root: &Path, violations: &mut Vec<GuardViolation>) {
    ...
    let stable_finalize_markers = [
        ...
        "contents: read",
        "actions: read",
        "gh release verify v0.4.3",
        "gh release verify-asset v0.4.3",
        "npm-candidate-manifest.json",
...
mod tests {
    ...
    fn stable_dist_tags_require_exact_latest_and_a_preserved_preview() {
        ...
        let published_stable = format!(
            r#"["{STABLE_PREVIEW_VERSION}","0.2.2","{STABLE_PREVIOUS_PUBLIC_VERSION}","{STABLE_RELEASE_VERSION}"]"#
        );
        assert_eq!(
        ...
        assert_eq!(
            release_dist_tag_action(
                ...
                "0.1.0-preview.9",
                STABLE_RELEASE_VERSION,
                r#"{"latest":"0.4.3","preview":"0.1.0-preview.9"}"#,
                r#"["0.1.0-preview.9","0.4.3"]"#,
            ),
            Err("preview does not match the required stable predecessor")
        );
        assert_eq!(
            release_dist_tag_action(
                ...
                "0.2.0-preview.0",
                "0.5.0",
                r#"{"latest":"0.5.0","preview":"0.2.0-preview.0"}"#,
                r#"["0.2.0-preview.0","0.5.0"]"#,
            ),
        ...
        for tags in [
            r#"{"latest":"0.4.3"}"#,
            r#"{"latest":"0.4.3","preview":"0.2.0-preview.0","beta":"0.4.3"}"#,
        ] {
            assert_eq!(
                release_dist_tag_action(
                    STABLE_RELEASE_VERSION,
    ...
    fn release_channel_and_dist_tag_commands_are_bounded() {
        ...
        let final_state = run(
            [
                ...
                "0.4.3",
                "--tags-json",
                r#"{"latest":"0.4.3","preview":"0.2.0-preview.0"}"#,
                "--versions-json",
                r#"["0.2.0-preview.0","0.2.2","0.4.0","0.4.1","0.4.2","0.4.3"]"#,
            ],
            Path::new("."),
        );
    ...
    fn stable_release_workflow_only_stages_after_draft_release_assets() {
        ...
        for invalid in [
            ...
            valid_release_workflow().replace(
                ...
                "npm stage inspect ./npm-candidate/sioyooo-repogrammar-0.4.3.tgz --access public --tag latest --provenance",
            ),
        ] {
            write_file(
                root.path().join(".github/workflows/release.yml"),
    ...
    fn stable_publication_authority_rejects_dynamic_or_comment_only_bypasses() {
        ...
        let dynamic = valid_release_workflow().replace(
            exact,
            &format!("# {exact}\n    npm \"${{subcommand}}\" ./npm-candidate/sioyooo-repogrammar-0.4.3.tgz --access public --tag latest --provenance"),
        );
        write_file(
            root.path().join(".github/workflows/release.yml"),
            dynamic.as_bytes(),
...
"#
                ...
                "dependency_uri" => {
                    statement["predicate"]["buildDefinition"]["resolvedDependencies"][0]["uri"] =
                        serde_json::Value::String(
                            "git+https://github.com/foreign/RepoGrammar@refs/tags/v0.4.3"
                                .to_string(),
                        );
                }
                "head_sha" => {
    ...
    fn stable_candidate_manifest() -> serde_json::Value {
        ...
        serde_json::json!({
            ...
            "filename": "sioyooo-repogrammar-0.4.3.tgz",
    ...
    fn stable_provenance_statement() -> serde_json::Value {
        serde_json::json!({
            ...
            "subject": [{
                "name": "pkg:npm/%40sioyooo/repogrammar@0.4.3",
    ...
    fn stable_audit_signatures(statement: &serde_json::Value) -> serde_json::Value {
        ...
        serde_json::json!({
            ...
            "verified": [{
                ...
                "attestations": {
                    "url": "https://registry.npmjs.org/-/npm/v1/attestations/@sioyooo%2frepogrammar@0.4.3",
    ...
    fn write_stable_evidence_fixture(root: &Path) {
        ...
        write_json(
            ...
            &serde_json::json!([
                STABLE_PREVIEW_VERSION,
                "0.2.2",
                STABLE_PREVIOUS_PUBLIC_VERSION,
                STABLE_RELEASE_VERSION
            ]),
        );
        write_json(
    ...
    /// Builds the product `repogrammar` binary from the current workspace and
    /// returns its exact path, reported by cargo itself. Self-building removes
    /// any dependency on a sibling artifact that a bare `cargo test` may or may
    /// not place next to the harness — a fresh CI checkout of
    /// `cargo test --locked --workspace --all-features` does not emit a
    /// standalone `target/<profile>/repogrammar` reachable from the deps-hashed
    /// harness path — so the end-to-end payload smoke always measures the
    /// current source instead of a stale or missing build. A build failure
    /// panics loudly with cargo's stderr rather than skipping the measurement.
    fn build_product_binary() -> PathBuf {
        let output = Command::new(env!("CARGO"))
            ...
            .output()
            .expect("spawn `cargo build --bin repogrammar` for the payload smoke");
        assert!(
            output.status.success(),
            "`cargo build --bin repogrammar` failed:\n{}",
            String::from_utf8_lossy(&output.stderr)
        );
        let stdout = String::from_utf8(output.stdout).expect("cargo build JSON stdout is UTF-8");
        ...
        for line in stdout.lines() {
            ...
            if message["reason"] == "compiler-artifact"
                && message["target"]["name"] == "repogrammar"
                && message["target"]["kind"]
                    .as_array()
        ...
        executable.expect("cargo build did not report a `repogrammar` bin executable path")
    }

    #[test]
    fn payload_measure_is_deterministic_and_schema_stable_end_to_end() {
        let binary = build_product_binary();
