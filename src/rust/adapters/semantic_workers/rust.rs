...
/// Cap on captured `cargo metadata` stdout. Real `--no-deps` output is a few MB
/// even for large workspaces; this bounds a pathological/hostile project so the
/// provider cannot exhaust host memory.
const MAX_CARGO_METADATA_BYTES: usize = 64 * 1024 * 1024;
/// Wall-clock bound so a hung `cargo metadata` cannot hang the host.
...
impl CargoMetadataRustProvider {
    ...
    pub fn analyze_project(
        ...
        let stdout_bytes = match self.run_cargo_metadata(project_root) {
            CargoMetadataRun::Succeeded(bytes) => bytes,
            CargoMetadataRun::Unavailable => {
                return Ok(RustProviderOutput::unavailable(
                    RustProviderKind::CargoMetadata,
                    RustProviderOperation::CargoProjectModel,
                ));
            }
            CargoMetadataRun::Failed => {
    ...
    /// Runs `cargo metadata` with a captured-output byte cap and a wall-clock
    /// timeout so a hostile or hung project cannot exhaust host memory or hang
    /// the caller. `stderr` is discarded; only bounded stdout is captured.
    fn run_cargo_metadata(&self, project_root: &Path) -> CargoMetadataRun {
        let mut child = match Command::new(&self.cargo_executable)
        ...
        {
            Ok(child) => child,
            Err(_) => return CargoMetadataRun::Unavailable,
        };
        let Some(stdout) = child.stdout.take() else {
            let _ = child.kill();
            let _ = child.wait();
            return CargoMetadataRun::Unavailable;
        };
        let reader = thread::spawn(move || read_capped(stdout, MAX_CARGO_METADATA_BYTES));
        ...
        let status = loop {
            ...
            if start.elapsed() >= CARGO_METADATA_TIMEOUT {
                let _ = child.kill();
                let _ = child.wait();
                let _ = reader.join();
                return CargoMetadataRun::Unavailable;
        ...
        match reader.join() {
            Ok(Ok(bytes)) if status.success() => CargoMetadataRun::Succeeded(bytes),
            Ok(Ok(_)) => CargoMetadataRun::Failed,
            // Output exceeded the cap or the pipe read failed.
            Ok(Err(())) | Err(_) => CargoMetadataRun::Unavailable,
        }
    }
}

enum CargoMetadataRun {
    Succeeded(Vec<u8>),
    Failed,
    Unavailable,
}

/// Reads `reader` fully into a byte vector, failing if the total exceeds `cap`.
...
mod tests {
  ...
  "version": 1
    ...
    #[test]
    fn read_capped_bounds_captured_output() {
        use std::io::Cursor;

        assert_eq!(
            read_capped(Cursor::new(vec![b'x'; 10]), 16),
            Ok(vec![b'x'; 10])
        );
        // Exactly at the cap is accepted.
        assert_eq!(
            read_capped(Cursor::new(vec![b'x'; 16]), 16),
            Ok(vec![b'x'; 16])
        );
        // One byte past the cap is rejected instead of buffered.
        assert_eq!(read_capped(Cursor::new(vec![b'x'; 17]), 16), Err(()));
    }
}
