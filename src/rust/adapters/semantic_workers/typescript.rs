...
impl TypeScriptSemanticWorkerBoundary {
    fn run_worker(&self, request: SemanticWorkerRequest) -> Result<String, SemanticWorkerError> {
        ...
        // Drain stdout/stderr concurrently and write stdin on its own thread so a
        // worker that emits output before draining its request cannot deadlock
        // the host. Writing the full request inline before spawning the readers
        // would block the host's `write_all` forever once the worker's stdout or
        // stderr pipe buffer fills, with no timeout covering the write.
        ...
        let stdin_writer = thread::spawn(move || {
            ...
            stdin.write_all(b"\n")
            // stdin drops here, closing the pipe so the worker observes EOF.
        });
        ...
        loop {
            if let Some(status) = child.try_wait().map_err(|_| {
                ...
                // The worker's exit status and output are authoritative; a
                // broken-pipe write (worker closed stdin early) is not a failure.
...
mod tests {
    ...
    fn process_boundary_preserves_timeout_after_child_exits_with_inherited_pipe() {
        ...
        let inherited_pipe = executable_script(
            &workspace,
            "inherited-pipe.sh",
            "#!/bin/sh\nexec 3>&1\n( trap '' HUP; /bin/sleep 1 ) >&3 2>&3 </dev/null &\nexec 3>&-\nexit 0\n",
        );
