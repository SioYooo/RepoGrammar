...
pub(super) fn anchor(
    ...
    if let Some((module, original)) = bindings.imported_runner(name) {
        ...
        {
            let (runner_kind, target) = runner_kind_and_target(module, original, is_suite);
...
/// Map an exact runner import module to its `runner_kind` provenance token and
/// the exact support target. `runner_kind` is a REQUIRED-equal family
/// compatibility feature so mocha, node:test, and jest/vitest suites never
/// cluster into the same family.
fn runner_kind_and_target(module: &str, original: &str, is_suite: bool) -> (&'static str, String) {
    match module {
        "mocha" => (
            "mocha",
            if is_suite {
        ...
        "@jest/globals" => ("jest", format!("jest_vitest.{original}")),
        _ => ("vitest", format!("jest_vitest.{original}")),
    }
}
