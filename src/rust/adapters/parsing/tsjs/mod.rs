...
pub fn exact_framework_anchors(
    ...
    let nest_context = nest::NestContext::analyze(&bindings, document.text, units);
    let mut facts = Vec::new();
    for unit in units {
        match anchor_for_unit(document, context, &bindings, &nest_context, unit) {
            AnchorOutcome::Anchor(anchor) => facts.push(anchor_fact(document, unit, anchor)?),
            AnchorOutcome::AnchorWithSubclaims(anchor, subclaims) => {
                for subclaim in subclaims {
                    facts.push(unknown::fact(document, unit, subclaim)?);
                }
                facts.push(anchor_fact(document, unit, anchor)?);
            }
...
enum AnchorOutcome {
    Anchor(Anchor),
    AnchorWithSubclaims(Anchor, Vec<UnknownAnchor>),
...
fn anchor_for_unit(
    ...
    match unit.kind {
        ...
        CodeUnitKind::ZodSchema => zod::anchor(bindings, slice, unit.range.start_byte),
        CodeUnitKind::NestController => {
            nest::controller_anchor(bindings, slice, unit.range.start_byte)
        }
        CodeUnitKind::NestRoute => {
            nest::route_anchor(bindings, nest_context, slice, unit.range.start_byte)
        }
        CodeUnitKind::NestInjectable => {
            nest::injectable_anchor(bindings, slice, unit.range.start_byte)
        }
        CodeUnitKind::NestModule => nest::module_anchor(bindings, slice, unit.range.start_byte),
        CodeUnitKind::HonoRoute => hono::anchor(bindings, slice, unit.range.start_byte),
...
"#;
    ...
    #[test]
    fn zod_exact_top_level_object_schemas_anchor_each_builder() {
        let text = r#"import { z } from "zod";
export const User = z.object({ id: z.string(), name: z.string() });
export const Account = z.object({ id: z.string(), balance: z.number() });
export const Order = z.object({ id: z.string(), total: z.number() });
"#;
        assert_eq!(
            targets("src/schemas.ts", text),
            vec![
                "zod.object".to_string(),
                "zod.object".to_string(),
                "zod.object".to_string(),
            ]
        );
        let facts = parse_facts("src/schemas.ts", text);
        let user = facts
            .iter()
            .find(|fact| {
                fact.target
                    .as_ref()
                    .is_some_and(|target| target.as_str() == "zod.object")
            })
            .expect("zod object fact");
        assert!(user
            .assumptions
            .iter()
            .any(|assumption| assumption == "zod_builder=object"));
        assert!(user
            .assumptions
            .iter()
            .any(|assumption| assumption.starts_with("zod_field_count_shape=")));
    }

    #[test]
    fn zod_alternate_builders_and_refinement_subclaim() {
        let text = r#"import { z } from "zod/v4";
export const Ids = z.array(z.string());
export const Choice = z.enum(["a", "b"]).refine((v) => v.length > 0);
"#;
        assert_eq!(
            targets("src/schemas.ts", text),
            vec!["zod.array".to_string(), "zod.enum".to_string()]
        );
        assert_eq!(
            unknown_kinds("src/schemas.ts", text),
            vec!["tsjs_zod_runtime_refinement".to_string()]
        );
    }

    #[test]
    fn zod_object_without_import_has_no_anchor() {
        let text = r#"const z = { object: (shape) => shape };
export const NotASchema = z.object({ id: 1 });
"#;
        assert!(targets("src/fake.ts", text).is_empty());
        assert!(unknown_kinds("src/fake.ts", text).is_empty());
    }

    #[test]
    fn nest_exact_controller_and_routes_anchor() {
        let text = r#"import { Controller, Get } from "@nestjs/common";
...
"#;
        assert_eq!(
            targets("src/users.controller.ts", text),
            vec![
                "nestjs.common.Controller".to_string(),
                "nestjs.common.Get".to_string(),
                "nestjs.common.Get".to_string(),
            ]
        );
        let facts = parse_facts("src/users.controller.ts", text);
        let route = facts
            .iter()
            .find(|fact| {
                fact.target
                    .as_ref()
                    .is_some_and(|target| target.as_str() == "nestjs.common.Get")
            })
            .expect("nest route fact");
        assert!(route
            .assumptions
            .iter()
            .any(|assumption| assumption == "http_method=get"));
        assert!(route
            .assumptions
            .iter()
            .any(|assumption| assumption == "route_path_shape=literal"));
        // Controller carries a non-blocking DI subclaim.
        assert_eq!(
            unknown_kinds("src/users.controller.ts", text),
            vec!["tsjs_nest_di_resolution".to_string()]
        );
    }

    #[test]
    fn nest_injectable_and_module_anchor() {
        let text = r#"import { Injectable, Module } from "@nestjs/common";

@Injectable()
export class UsersService {}

@Module({ providers: [UsersService], controllers: [], imports: [] })
export class UsersModule {}
"#;
        assert_eq!(
            targets("src/users.module.ts", text),
            vec![
                "nestjs.common.Injectable".to_string(),
                "nestjs.common.Module".to_string(),
            ]
        );
        let facts = parse_facts("src/users.module.ts", text);
        let module = facts
            .iter()
            .find(|fact| {
                fact.target
                    .as_ref()
                    .is_some_and(|target| target.as_str() == "nestjs.common.Module")
            })
            .expect("nest module fact");
        assert!(module
            .assumptions
            .iter()
            .any(|assumption| assumption.starts_with("nest_module_shape=providers_1_")));
    }

    #[test]
    fn nest_lookalike_without_import_stays_blocking_unknown() {
        let text = r#"function Controller(path) { return (target) => target; }
function Get(path) { return (target, key) => key; }
...
"#;
        assert!(targets("src/fake.controller.ts", text).is_empty());
        assert_eq!(
            unknown_kinds("src/fake.controller.ts", text),
            vec![
                "nest_unresolved_controller_import".to_string(),
                "nest_unresolved_route_import".to_string(),
            ]
        );
    }

    #[test]
    fn hono_exact_routes_anchor_each_literal_get() {
        let text = r#"import { Hono } from "hono";
const app = new Hono();
...
"#;
        assert_eq!(
            targets("src/routes.ts", text),
            vec![
                "hono.route.get".to_string(),
                "hono.route.get".to_string(),
                "hono.route.get".to_string(),
            ]
        );
        let facts = parse_facts("src/routes.ts", text);
        let route = facts
            .iter()
            .find(|fact| {
                fact.target
                    .as_ref()
                    .is_some_and(|target| target.as_str() == "hono.route.get")
            })
            .expect("hono route fact");
        assert!(route
            .assumptions
            .iter()
            .any(|assumption| assumption == "http_method=get"));
    ...
    #[test]
    fn hono_untraced_receiver_is_not_a_hono_anchor() {
        let text = r#"const router = makeRouter();
router.get("/users", (c) => c.json([]));
"#;
        assert!(targets("src/routes.ts", text).is_empty());
        assert_eq!(
            unknown_kinds("src/routes.ts", text),
            vec!["unresolved_express_receiver".to_string()]
        );
    }

    #[test]
    fn mocha_and_node_test_named_imports_alias_runner_kind() {
        let mocha = r#"import { describe, it } from "mocha";
describe("users", () => {
  it("loads", () => {});
});
"#;
        assert_eq!(
            targets("src/users.test.ts", mocha),
            vec!["mocha.describe".to_string(), "mocha.it".to_string()]
        );
        let facts = parse_facts("src/users.test.ts", mocha);
        ...
        let node = r#"import { test, describe } from "node:test";
describe("orders", () => {
  test("creates", () => {});
});
"#;
        assert_eq!(
            targets("src/orders.test.ts", node),
            vec![
                "node_test.describe".to_string(),
                "node_test.test".to_string(),
            ]
        );
        let node_facts = parse_facts("src/orders.test.ts", node);
        assert!(node_facts.iter().any(|fact| {
            fact.assumptions
                .iter()
                .any(|assumption| assumption == "runner_kind=node_test")
        }));
    }
}
