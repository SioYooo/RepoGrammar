...
grep -q -- "--version v0.3.2" "$NO_RELEASE_ERR"
...
grep -q 'npm view @sioyooo/repogrammar@0.3.2 version' "$README_FILE"
grep -q 'releases/download/v0.3.2/install.sh.sha256' "$README_FILE"
grep -q 'npx @sioyooo/repogrammar@0.3.2 setup' "$README_FILE"
...
require_workflow_match "$STAGE_STABLE_JOB" '^[[:space:]]+npm stage publish \./npm-candidate/sioyooo-repogrammar-0\.3\.2\.tgz --access public --tag latest --provenance' \
...
require_workflow_match "$STABLE_FINALIZER_JOB" '^[[:space:]]+gh release verify v0\.3\.2' \
  "stable finalization must verify the immutable release attestation"
require_workflow_match "$STABLE_FINALIZER_JOB" '^[[:space:]]+gh release verify-asset v0\.3\.2' \
...
require_workflow_match "$STABLE_FINALIZER_JOB" '@sioyooo/repogrammar@0\.3\.2' \
