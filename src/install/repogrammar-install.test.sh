...
grep -q -- "--version v0.3.1" "$NO_RELEASE_ERR"
...
if [[ "$PACKAGE_VERSION" != "0.3.1" || "$CARGO_VERSION" != "$PACKAGE_VERSION" ]]; then
  echo "stable manifests must agree on 0.3.1" >&2
  exit 1
fi
...
grep -q 'npm view @sioyooo/repogrammar@0.3.1 version' "$README_FILE"
grep -q 'releases/download/v0.3.1/install.sh.sha256' "$README_FILE"
grep -q 'npx @sioyooo/repogrammar@0.3.1 setup' "$README_FILE"
...
require_workflow_match "$STAGE_STABLE_JOB" '^[[:space:]]+npm stage publish \./npm-candidate/sioyooo-repogrammar-0\.2\.2\.tgz --access public --tag latest --provenance' \
...
require_workflow_match "$STABLE_FINALIZER_JOB" '^[[:space:]]+gh release verify v0\.2\.2' \
  "stable finalization must verify the immutable release attestation"
require_workflow_match "$STABLE_FINALIZER_JOB" '^[[:space:]]+gh release verify-asset v0\.2\.2' \
...
require_workflow_match "$STABLE_FINALIZER_JOB" '@sioyooo/repogrammar@0\.2\.2' \
