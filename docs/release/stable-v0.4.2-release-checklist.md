# Stable v0.4.2 Release Checklist

This is the canonical two-phase publication gate for RepoGrammar `0.4.2`.
It patch-forwards from the immutable public `0.4.1` release and makes the
judge-facing installation lifecycle explicit: the public shell installer owns
binary acquisition only, coding-agent integration is optional, and `init` owns
each repository's local state, index, and autosync.
...
- Cargo, Cargo lockfile, and npm manifest versions are exactly `0.4.2`.
- The publication tag is annotated `v0.4.2` at the exact fetched
  `origin/main` commit after all local gates pass.
- The retained npm candidate is exactly
  `sioyooo-repogrammar-0.4.2.tgz` and is never repacked before approval.
- The final public dist-tags must be exactly `latest=0.4.2` and
  `preview=0.2.0-preview.0`.
- Public inventory must retain `0.2.0-preview.0`, `0.2.2`, `0.4.0`, `0.4.1`,
  and `0.4.2`; abandoned `0.2.0`, `0.2.1`, `0.3.0`, `0.3.1`, and `0.3.2`
  remain absent.
- Any source correction after tag creation consumes `0.4.2` and requires a new
...
- Fetch `origin/main`, prune, and fetch tags. Require a clean worktree and
  `HEAD == origin/main`.
- Require no local/remote `v0.4.2` tag, no GitHub release or draft for the tag,
  no public npm `0.4.2`, and no retained private npm stage for that version.
  Public registry metadata cannot prove private stage absence; the maintainer
  checks authenticated stage state separately.
- Preserve the immutable `v0.4.1` tag, assets, npm package, provenance, and
...
- release workflow, finalizer, repository guard, npm launcher, installer hint,
  README, and canonical docs all agree on `0.4.2`.

## Candidate tag workflow

1. Create annotated `v0.4.2` at exact `origin/main` and push only that tag.
...
- Approve the exact npm `0.4.2` stage through the maintainer 2FA boundary.
