# Stable v0.2.1 Release Checklist

This checklist is the canonical gate for RepoGrammar `0.2.1`, the intended
first published stable-channel pre-1.0 release. It makes `0.2.1` the npm
...
`@sioyooo/repogrammar@0.2.0` package were published. That source defect requires
this `0.2.1` patch-forward; the `v0.2.0` tag must never be moved, deleted, or
reused, and rehearsal artifacts from that attempt are not publication
authority.
...
- Cargo, Cargo lockfile, and npm manifest versions are exactly `0.2.1`.
- The publication candidate tag is `v0.2.1`, created at the exact current `origin/main`
  commit after all gates pass.
- npm `@sioyooo/repogrammar@0.2.1` is a new immutable version. The existing
  `0.2.0-preview.0` tarball and `preview` dist-tag are never replaced,
  unpublished, or repurposed.
- The expected final dist-tags are `latest=0.2.1` and
  `preview=0.2.0-preview.0`.
- Any correction after public npm approval is a patch-forward `0.2.2`; never
  reuse `0.2.1`, move `v0.2.1`, or replace immutable release assets. The failed
  `v0.2.0` tag is retained and non-reusable under this release policy.
...
The pushed `v0.2.1` tag run is the sole publication-candidate build. Before it
...
- one exact `sioyooo-repogrammar-0.2.1.tgz` plus its integrity and exact file
  manifest.
...
1. Keep GitHub Immutable Releases enabled for `SioYooo/RepoGrammar`. It was
   enabled before the failed `v0.2.0` tag run and applies to `v0.2.1`.
...
2. Create and push `v0.2.1`. The tag run is the sole candidate run: it rebuilds
...
- GitHub `v0.2.1` is public, non-prerelease, immutable, and contains exactly 11
  assets: four supported archives, their checksums, `install.sh`, its checksum,
  and `npm-candidate-manifest.json`.
- npm published versions contain both `0.2.0-preview.0` and `0.2.1`, and do not
  contain the failed, unpublished `0.2.0` candidate. Its presence fails closed.
- npm dist-tags are exactly compatible with `latest=0.2.1` and
  `preview=0.2.0-preview.0`.
...
- The downloaded public x86_64 Linux archive passes the full packaged-artifact
  ...
  installed command reports `repogrammar 0.2.1`.
- Pinned and unversioned public `npx` paths report `0.2.1` and each completes a
...
|---|---|
| Candidate run failed before a draft exists | Rerun only for an external/transient failure. A source correction is `v0.2.2`; never move `v0.2.1` or manufacture authority from rehearsal artifacts. |
...
| npm approved, final verification failed | `0.2.1` is consumed forever; deprecate if necessary and fix forward in `0.2.2`. |
