...
- preview staging has one registered assignment for its exact
  ...
  literal command for `./npm-candidate/sioyooo-repogrammar-0.3.0.tgz`; a bare
...
`@sioyooo/repogrammar@0.3.0` entry from the exact registry and exactly one SLSA
...
`.github/workflows/release.yml`, the push tag to `refs/tags/v0.3.0`, the
...
- the registered stable `0.3.0` policy requires exact `latest=0.3.0`, exact
...
`latest=0.3.0`/`preview=0.2.0-preview.0`. All inconsistent states fail visibly
