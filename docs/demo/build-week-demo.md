...
- [ ] If the no-build install is shown, the exact GitHub asset and npm package
      have passed the stable `0.2.1` release checklist. Until then, record the
...
After `v0.2.1` artifacts and npm publication are independently
verified, replace that acquisition shot with the no-build path:

```bash
npx @sioyooo/repogrammar@0.2.1 setup --project "$DEMO_REPO" --target codex
```
