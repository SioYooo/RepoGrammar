...
- [ ] If the no-build install is shown, the exact GitHub asset and npm package
      have passed the public-preview release checklist. Until then, record the
...
After `v0.2.0-preview.0` artifacts and npm publication are independently
verified, replace that acquisition shot with the no-build path:

```bash
npx @sioyooo/repogrammar setup --project "$DEMO_REPO" --target codex
```
