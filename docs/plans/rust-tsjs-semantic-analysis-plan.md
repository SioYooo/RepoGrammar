...
repo-local Express/Fastify named handler imports, Prisma shared-client
imports, or Drizzle db/table imports; matching `resolve_export` or
...
name, framework role, every required import specifier where relevant, and
structural-anchor provenance. Without that API, the worker
...
2. Bounded TypeScript worker operation slice for module/import/export/package
   ...
   shared-client binding, plus relative repo-local Drizzle db/table binding;
   broader Program construction
   remains future work.
