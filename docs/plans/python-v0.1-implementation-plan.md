...
private project-config summaries for root `pyproject.toml`/`tomllib`,
`setup.cfg`/`configparser`, and static import-bound `setuptools`
`setup.py`/CPython-AST with strict complete call/mapping shapes and typed
incomplete-config abstention, plus semantic-
worker-compatible project-mode module-level repo-local import resolution are
implemented. Default indexing now passes discovered repo-relative `.py`
inventory, bounded module file texts, a deduplicated structural candidate union
of sanitized roots from those project-config facts (not packaging precedence),
and bounded discovered `conftest.py` context into private
...
factories, Pydantic runtime validator side effects, imported external
Pydantic/SQLAlchemy bases, dynamic imports including `__import__` and
...
mutation, dynamic call target, dynamic/unresolved decorator, dynamic Pydantic
...
metadata-only context/effect facts rather than family support, and custom query
wrapper calls remain typed UNKNOWNs instead of becoming repository support.
...
Current UNKNOWN policy keeps Python call-target claim impact separate from
recoverability. An exact eval/exec/getattr/namespace or other typed runtime
boundary remains irreducible, while a call target without that assumption stays
a provider-resolvable dispatch obligation for future Pyrefly/Pyright work. The
legacy public `UnknownClass` and inventory counters remain unchanged.
