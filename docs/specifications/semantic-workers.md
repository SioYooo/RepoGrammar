...
## Go preflight strategy

ADR-0021 accepts a future Go standard-library semantic worker design but adds
no Go runtime support. The authoritative path must be explicit, opt-in, and
sandboxed, consume supplied inputs where possible, and use `go/parser`,
`go/token`, candidate-scoped `go/types` with a controlled importer, and
`go/build/constraint`. A later version-pinned Tree-sitter Go integration may
generate fallback syntax candidates only; it is not authorized as a dependency
by the preflight and cannot support a family alone.

The safe default must not run `go/packages`, `go list`, gopls, cgo, or target-
repository build, test, run, or generate commands, read target-repository/home/
credential/cache files, or use an ambient importer. Source arrives only through
the bounded request; ranges use `go/token` unadjusted byte offsets and never
`//line`-adjusted filenames. Missing consent or sandbox
capability, worker/version failure, resource exhaustion, build/config/workspace
ambiguity, generated files, external type facts, dispatch, cgo, and generators
remain typed `UNKNOWN` for affected claims. The exact limits, first
`go.testing.test_function` family, cache/provenance manifest plus worker digest,
and atomic module gates are normative in
`docs/decisions/ADR-0021-go-standard-library-semantic-worker-preflight.md`.
The current TypeScript worker process boundary is not the required Go sandbox;
Go execution remains unavailable until a separately reviewed OS boundary proves
network, filesystem, descendant-process, timeout, CPU, and memory isolation.
The Go protocol must also distinguish encoded-wire limits from decoded-source
limits and reject an oversized batch before spawning; it must not accept partial
facts after truncation, timeout, or resource exhaustion.

## PHP preflight strategy

ADR-0024 accepts a future PHP worker design and adds no PHP semantic runtime
support or dependency. PHP's separate discovery-only inventory does not invoke
or admit a worker. The production frontend candidate is `mago-syntax`
1.43.0 in a separately reviewed OS-sandboxed worker over supplied bytes.
Official PHP 8.5.8 `php -n -l` is restricted to the isolated syntax-validity
oracle role. `nikic/PHP-Parser` 5.8.0 is the isolated AST/location differential
and separately qualification-gated fallback. Tree-sitter PHP 0.24.2 may
generate syntax candidates only. None is authorized for production by the
preflight.

The future frontend worker may receive one bounded PHP file as supplied bytes
plus one bounded allowlisted PHP profile summary; it must not receive raw
configuration or traverse repository, home, or cache state. A separate future
bounded non-executing project-model parser may receive raw Composer JSON/lock
and PHPUnit XML bytes, pin Composer 2.10.2 content-hash semantics, and emit only
the normalized allowlisted summary. Neither path may execute Composer, PHPUnit,
autoloaders, plugins, scripts, repository PHP, target dependencies, child
processes, or network access. The exact `php.phpunit.test_method` family, future
claim-impact obligations, resource/protocol limits, dependency gates, and
atomic stages are normative in
`docs/decisions/ADR-0024-php-sandboxed-frontend-phpunit-preflight.md`. PHP is
`discovered_only`, and semantic-worker capability is unavailable until the
required implementation and final completion audit land.
...
conditional selection, macro expansion, protocol conformance, dispatch,
buildability, or runtime test discovery.

An exact Swift 6.3.3 SourceKit-LSP/sourcekitd/compiler path is only a separately
qualified semantic identity candidate for the future direct XCTest slice. It
may receive one supplied Swift file plus one bounded normalized project/profile
summary in a synthesized workspace backed only by immutable admitted
toolchain/SDK/XCTest artifacts. It must not open the target repository, evaluate
ordinary or version-specific `Package.swift`, run SwiftPM/Xcode/build/test,
enable background indexing, resolve dependencies, load target modules,
macros/plugins/generators, use ambient caches/credentials/toolchain state,
...
is `discovered_only`. Swift Testing remains deferred because `@Test` is a
compiler macro. Swift semantic
capability remains unavailable until the required implementation and final
completion audit land.
`docs/plans/swift-n1-qualification-handoff.md` records the paused discovery
baseline and the next documentation/evidence-only artifact, differential,
dependency, target, and native-sandbox qualification. A positive stage-3
verdict does not admit a worker or dependency; those remain a separate atomic
stage.

## Ruby preflight strategy

ADR-0022 accepts a future Ruby Prism design but adds no Ruby runtime support or
dependency. The candidate `ruby-prism` 1.9.0 wrapper consumes supplied bytes,
but it links native C99 Prism through FFI and the exact upstream commit/package
source lacks the syntax-version options shown by mutable hosted rustdoc. A
documentation/evidence-only dependency and sandbox qualification must pass
before production dependency/artifact admission. The future frontend must use
that accepted exact-artifact API and explicit CRuby 4.0 profile in a separately
reviewed OS-sandboxed worker; it must not link the native parser into the
primary process for untrusted input.

The first profile is selected only by a sole repository-root `.ruby-version`
whose complete content is exact `4.0.6` plus optional LF. A bounded project-
model step validates it. Nested, absent, alternate, or conflicting forms must
become `ruby_syntax_version` `UNKNOWN` after the obligation registry lands;
until then Ruby semantic capability is unavailable. The worker receives only
bounded `.rb` source bytes plus normalized validated profile metadata, never
raw Gemfile, gemspec, Bundler configuration, lockfile, or `.ruby-version` bytes.

The safe default must not traverse repository/home/cache state or execute Ruby,
Bundler, RubyGems, Rake, Rails, tests, generators, project manifests, installed
gems, child processes, or network access. Missing artifact/profile/sandbox
capability, parser degradation, resource failure, ambiguous engine/version,
loading/constant identity, and runtime mutation must become typed `UNKNOWN` for
affected claims after the registry lands. Generated-source blocking additionally
requires bounded positive generated/conflicting-origin evidence; lack of an
origin marker alone does not block an ordinary `.rb` file. The exact
`ruby.minitest.test_method` slice, proposed CPU/thread/other limits, dependency
gates, obligation names, and ten atomic stages are normative in
...
the worker's own trusted install, the worker may use the compiler API's module
resolver and source-file parser for exact export identity, including
`resolve_reexport` operations that prove both a repo-local module target and
named export for a bounded source anchor, then emit `SEMANTIC` facts with
`provider=typescript`, `provider_resolved=true`, `query_operation=<operation>`,
config/package hashes, and a safe environment fingerprint. The worker must not
load the analyzed repository's own `typescript` package by default, because
`require()`-ing it runs that package's module code in-process — arbitrary code
execution on an untrusted repository. Loading the project's TypeScript is
opt-in only, gated behind `REPOGRAMMAR_TSJS_TRUST_PROJECT_TYPESCRIPT=1` for
repositories the operator explicitly trusts. TypeScript config parsing
(including `tsconfig` `extends`) is bounded to the project root so a hostile
`extends` target cannot read files outside the repository. When no TypeScript API is available, the
...
request object.
The adapter must drain worker stdout and stderr concurrently while it writes the
request to stdin (for example, by writing stdin on a dedicated thread). Writing
the whole request before starting to read would deadlock the host if the worker
emits output before draining its request: a full stdout/stderr pipe blocks the
worker's write while it never reads stdin, and the host's blocking write would
then hang with no timeout covering it.
...
Optional provider subprocesses that capture output (such as the off-default
`cargo metadata` Rust provider) must bound captured stdout and enforce a
wall-clock timeout so a hostile or hung project cannot exhaust host memory or
hang the caller; exceeding either bound becomes a provider-unavailable outcome.
...
Express/Fastify route-handler, Prisma shared-client, or Drizzle db/table
binding requests encoded as `<specifier>#<export>` for relative repo-local
named imports such as `./handlers#listUsers`, `./db#prisma`, `./db#db`, or
`./schema#users`. Returned facts are sorted
...
TypeScript-provider `resolve_reexport` fact can feed Express/Fastify handler,
Prisma, or Drizzle derived support only when it matches a provider-required
parser anchor's same path, hash, code unit, range, framework role, relative
import specifier, and export name. For Drizzle imported db/table anchors, every
required binding for that parser anchor must have a TypeScript-provider
`resolve_reexport` match before support is derived.
...
nested `Config`, dynamic `ConfigDict` UNKNOWNs, `computed_field`, validator,
and `model_validator`
...
strings or imported `text(...)` SQL text, and bounded
...
UNKNOWNs, SQLAlchemy event-listener UNKNOWNs, simple call targets, bounded
...
Pydantic `ConfigDict` values, dynamic SQLAlchemy relationship targets, raw SQL
query shapes, runtime-injected SQLAlchemy session receivers, dynamic pytest
fixture names, SQLAlchemy event listeners, dynamic SQLAlchemy model classes,
...
positional-or-keyword `where`, never both, with no unpacking. Same-leaf
local/helper calls, standalone/lookalike finders, conditional/dead calls,
and shadowed/deleted/mutated bindings (including builtins-qualified helpers)
abstain. A recognized call with computed, incomplete, duplicate, unpacked,
