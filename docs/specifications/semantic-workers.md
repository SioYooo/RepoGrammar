...
anchors, class bases, simple call targets, `pytest.test` test-function anchors,
same-file pytest fixture edges, path-derived module names, and CPython
`symtable` scope anchors, plus typed `UNKNOWN` facts for dynamic import,
unresolved import, framework magic, and unresolved pytest fixture injection
cases. Default indexing passes the
