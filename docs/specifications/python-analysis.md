...
- product CLI smoke tests that copy those fixtures into temporary repositories,
  ...
  exact FastAPI, pytest, and Pydantic anchors can produce default families only
  through separate derived support facts, and keep the test-injected `SEMANTIC`
  worker fixture as coverage for the explicit worker boundary;
