...
RepoGrammar `0.3.0` is the stable-channel candidate. Its MCP API
...
- Stable artifacts are available only after the exact `v0.3.0` GitHub release
  is public and immutable and npm `@sioyooo/repogrammar@0.3.0` is independently
  verified.
...
- The two-phase immutable rollout and recovery states are tracked in
  `release/stable-v0.3.0-release-checklist.md`. The earlier preview record
...
- **Token-saving readiness caps at partial.** The `token_saving_readiness`
  signal reports at most `partial` in `0.3.0`; a dedicated `ready`
  band is deferred.
