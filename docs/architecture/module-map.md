...
| `src/rust/application/` | Indexing, query, conformance, install planning, receipted machine-product uninstall, zero-friction setup orchestration, progress, repository lifecycle, auto-sync lifecycle state, storage, telemetry, metrics, proof-lattice support-promotion, and authoritative readiness/UNKNOWN recovery-policy use-case boundaries | `docs/architecture/overview.md`, this file, relevant specifications |
| `src/rust/application/install.rs` | Agent install and receipt-owned disconnect transactions, including native/instruction snapshots and rollback | `docs/specifications/installation.md`, `docs/specifications/cli.md` |
| `src/rust/application/product_installation.rs` | Atomic product receipt, strict first-party layout validation, and conservative legacy ownership inference | `docs/decisions/ADR-0028-receipted-product-self-uninstall.md`, `docs/specifications/installation.md` |
| `src/rust/application/product_uninstall.rs` | Private cleanup-plan validation, post-exit helper protocol, deletion-time ownership checks, and structured complete/partial reporting | `docs/decisions/ADR-0028-receipted-product-self-uninstall.md`, `docs/specifications/installation.md` |
| `src/rust/application/query.rs`, `query_terms.rs` | Exact-first family query routing, deterministic vocabulary-versioned term normalization/ranking, calibrated abstention, bounded freshness hydration, and static-alignment orchestration | `docs/specifications/query-resolution.md`, `docs/specifications/domain-model.md` |
