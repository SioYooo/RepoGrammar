...
- `index`, `sync`, and `resync` must acquire `.repogrammar/locks/index.lock`
  ...
  the shared application process-liveness policy on Windows and Unix before
  replacing stale locks, must reject impossible current-OS PID values before
  probing, must treat newer Unix process start time as PID reuse when the
  platform exposes it, and may remove only the lock bytes they wrote.
  `unlock --force --yes` may remove
...
- Auto-sync daemon acquisition must write complete `.repogrammar/locks/daemon.lock`
  ...
  when the inspected bytes still match. Autosync status and repository readiness
  consider a daemon lock running only when the recorded PID is live and, on
  Unix, its command line confirms `repogrammar autosync run`. A new daemon start
  becomes ready only after a bounded parent observes its expected child PID and
  startup nonce in the acquired lock while the child remains alive. PID
  allocation alone is insufficient; exit, lock refusal, and timeout are typed
  failures. Stop and guard cleanup must compare the full inspected lock bytes
  before removal.
- Agent integration ownership distinguishes current authority, internally
  consistent obsolete authority, foreign, malformed, and receipt/native drift.
  Obsolete RepoGrammar-owned entries may be refreshed only through the install
  service's snapshot/restore reconciliation. Setup rollback may remove only
  newly configured targets; it must never delete a pre-existing owned target
  merely because that target was refreshed in the same setup run.
- Setup readiness must keep product MCP self-test, native ready/blocked agent
  targets, repository index, auto-sync, and family evidence as separate facts.
  Family inventory failure is `Unknown`, not zero. A coding-agent question is
  allowed only when at least one native integration is verified ready.
